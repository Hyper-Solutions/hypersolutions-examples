...
//   - Not follow redirects (to detect Incapsula challenges)
...
func createHTTPClient(config *Config) (tlsclient.HttpClient, error) {
	...
	options := []tlsclient.HttpClientOption{
		...
		// Don't follow redirects - we need to detect Incapsula challenge redirects
...
func (s *Reese84Solver) getPow(ctx context.Context) (string, error) {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", powURL, strings.NewReader(powBody))
	if err != nil {
		return "", err
	}
...
func (s *Reese84Solver) submitSensor(ctx context.Context, sensor string) error {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", sensorURL, strings.NewReader(sensor))
	if err != nil {
		return err
	}
	...
	s.client.SetCookies(cookieURL, []*http.Cookie{
		{
			Name:   "reese84",
			Value:  result.Token,
